...
class HeightMap:
  ...
  def getCroped(self,x,z,w,h):
    return HeightMap(w,h,[
      self.getHeight(x + n % w,z + int(n/w)) for n in range(w*h)
    ])
  ...
  def getResized(self,newW,newL,method=lambda x,z,map: map.getHeight(x,z)):
    ...
    for i in range(len(newMap)):
      ...
      if(p != lp):
        print("[{}{}] - {:>8}/{:<8}".format("="*p," "*(25-p),i+1,len(newMap)),end="\r")
      lp = p
    print("\nDone")
...
def createWeightGrid(size):
  if(not int(size) in w_cache):
    c = size/2
    weights = [size-((c-n%size)**2 + (c-int(n/size))**2)**0.5  for n in range(0,size*size)]
