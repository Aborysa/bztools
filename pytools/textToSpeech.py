...
def espeakSay(text, voice="en", out="tmp.wav", speed=175):
    subprocess.call(
        'espeak "{}" -v {} -s {} -w {}'.format(text, voice, speed, out), shell=True
    )
    return "wav"
...
if os.path.isfile(args.path):
    ...
    files = [
        n for n in glob.glob("{}/*.txdi".format(args.path)) if not is_binary_file(n)
    ]
                ...
                if len(s) > 1:
                    ...
                    for i in range(0, len(segments), 2):
                        if segments[i].strip():
                            cmds.append(
                                {
                                    "voice": context[person],
                                    "text": segments[i].strip(),
                                    "delay": int(segments[i + 1])
                                    if (i + 1) < len(segments)
                                    else 0,
                                }
                            )
